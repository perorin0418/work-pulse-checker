...
## 追記: 「プロセスは生きているのに監視だけ止まる」再発の原因調査（2026-08-25）

上記のウォッチドッグ導入後も、プロセス自体は起動したままなのに監視（サンプラー／スケジューラ）が止まる事象が実機で再発した。原因は2点あり、両方ともウォッチドッグ自身の耐性不足に起因する。

### 原因1: `std::thread::spawn` は失敗すると呼び出し元をパニックさせる

`start_sampler` / `start_scheduler` は素の `thread::spawn` を使っていた。`thread::spawn` はOSがスレッドを作れないとき（ハンドル数上限到達など）内部で `.expect(...)` してパニックする。ウォッチドッグは stale 検知のたびにこの関数を**自分のループの中で無条件に**呼んでいたため、スレッド生成失敗の瞬間にウォッチドッグスレッド自体が死ぬ。ウォッチドッグのループ本体は `catch_unwind` で包まれておらず、この panic は誰にも捕まらない。以後 stale を検知する者がいなくなり、監視は永久に止まる。

### 原因2: Win32呼び出しが稀にハングし、スレッドがリークする

`windows_activity::active_window` は `GetWindowTextW` / `GetWindowTextLengthW` で前面ウィンドウのタイトルを読む。ドキュメント上は「対象が別プロセスのウィンドウなら `WM_GETTEXT` を送信しない」とされるが、実機ではサブクラス化されたウィンドウや一部のUWP/通知ウィンドウに対してハングする既知の問題がある。ハングしたサンプラースレッドはウォッチドッグの世代番号チェックにより「無害化」されるが、**OSスレッドとしては永久にブロックされたまま残り続ける**（世代チェックは新しいスレッドが古いスレッドを無視させるだけで、ブロックしたスレッド自体を終了させる手段がない）。

この2つが組み合わさると、Win32呼び出しのハングが起きるたびにスレッドがリークし → いずれOSのスレッド/ハンドル数上限に近づき → `thread::spawn` が失敗してパニックし → 原因1によりウォッチドッグ自体が沈黙する、という経路で「プロセスは生きているが監視だけ止まる」が再発する。

### 対策

1. **`resilience::spawn_resilient`** を追加。`thread::Builder::spawn` の `Result` をログに落として呑み込み、スレッド生成に失敗してもパニックさせない。`start_sampler` / `start_scheduler` / ウォッチドッグ自身の起動をすべてこれに差し替えた。
2. **ウォッチドッグのループ本体を `guarded` で包む**。既存の `guarded`（パニックを `catch_unwind` で飲み込むラッパー）を1巡分の処理に適用し、万一の panic でもウォッチドッグのループ自体は継続する。
3. **`windows_activity::read_window_title` を `GetWindowTextW`/`GetWindowTextLengthW` から `SendMessageTimeoutW`（`SMTO_ABORTIFHUNG`、タイムアウト300ms）へ置き換え**。対象ウィンドウが応答しない場合は打ち切られて0を返すため、原因2そのもの（Win32呼び出しによるスレッドのハング）を構造的に起こさせなくする。これによりスレッドリークの発生源を断ち、原因1が誘発される経路自体を塞ぐ。

### テスト

- `spawn_resilient` が渡した処理を別スレッドで実行することを確認するユニットテストを追加（`resilience.rs`）
- `SendMessageTimeoutW` へのWin32呼び出しそのものは実機依存のためユニットテスト化していない。手動確認として、応答しないウィンドウ（例: フリーズさせたプロセスの前面化）に対してもサンプラーが3秒間隔で動き続けることを確認する
