...
監視ログが1件も無い日を選んだ場合は、監視タスクが動く時間帯（既定 7:00〜22:00）の
空の30分枠を並べて一から作成できる。この範囲は `schedule.py` に1か所だけ定義があり、
タスクスケジューラーの登録内容（`StartBoundary` と `Duration`）と常に一致する。
内容を入力した枠だけがファイルに保存され、保存後も枠の番号は変わらないので
続けて他の時間帯を入力できる。
