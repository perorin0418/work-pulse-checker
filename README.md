...
作業サマリーの各行（`confirmed_text` ごとの合計時間）が1レコードに対応し、`業務種別` は固定値「直接原価」、`作業内容` に `confirmed_text`、`作業時間` に `HH:MM` 形式の合計時間が入る。それ以外の項目は現状自動入力できないため空文字になる（手動で埋めて日報アプリに転記する想定）。
